<template>
  <div class="min-h-screen bg-[#f7f7f7]" dir="rtl" @scroll.passive="onScroll" ref="page">
    ...
    <div class="w-full bg-white border-b border-gray-200 fixed top-0 left-0 right-0 z-50">
      <div class="h-12 px-2 flex items-center justify-between">
        ...
        <div class="flex-1 px-1">
          <div class="flex items-center bg-gray-100 rounded-full h-9 px-2 cursor-pointer" @click="returnToSearch">
            <input 
              v-model="headerSearchQ" 
              readonly
              placeholder="بحث" 
              class="flex-1 bg-transparent text-[12px] text-gray-800 outline-none cursor-pointer" 
            />
            <div class="flex items-center gap-1">
              <button aria-label="مسح" class="w-7 h-7 flex items-center justify-center" @click.stop="clearSearch">
      ...
      <!-- منطقة الفئات (تظهر الفئات المرتبطة بالبحث) -->
    ...
    <section class="px-1 pb-1">
      ...
      <!-- شبكة المنتجات -->
      <div v-else-if="products.length" class="product-grid grid grid-cols-2 gap-x-[5px] gap-y-0 grid-flow-row-dense">
        <div>
...
function applySearch(){
  const q = String(headerSearchQ.value||'').trim()
  if (q){
    router.push({ query: { q } })
  }
}
...
function returnToSearch() {
  // Return to search page with current query to show suggestions
  router.push({ path: '/search', query: { q: headerSearchQ.value } })
}

function clearSearch() {
  // Return to search page with empty query (clean state)
  router.push('/search')
}
...
async function loadCategories(){
  try{
    ...
    // Filter categories based on products found
    // This is a client-side approximation since we don't have a facets API
    if (products.value.length > 0) {
        const productCategoryIds = new Set(products.value.map(p => p.categoryId).filter(Boolean))
        // Also include parent categories
        const relevantCategories = allCategories.value.filter(c => productCategoryIds.has(c.id) || productCategoryIds.has(c.slug))
        ...
        categories.value = relevantCategories.map(c=> ({
            id: c.slug||c.id,
            label: c.name,
            img: safeImg(c.image)
        }))
...
async function loadProducts(limit: number = 10){
  try{
    ...
    const mappedRaw = items.map((it:any)=> ({
      ...
      overlayBannerSrc: it.overlayBannerSrc || undefined,
      overlayBannerAlt: it.overlayBannerAlt || undefined,
      categoryId: it.categoryId || it.category?.id, // Capture category ID for filtering
      categoryIds: Array.isArray(it.categoryIds) ? it.categoryIds : undefined,
      _ratio: undefined,
      _imgLoaded: false
    }))
