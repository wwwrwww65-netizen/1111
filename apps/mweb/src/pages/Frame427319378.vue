<template>
  <div class="container" style="max-width:430px;margin:0 auto;min-height:100vh">
    <div style="position:absolute;left:-72px;top:9960px;width:42px;height:49px">
      <div style="position:absolute;left:-67px;top:9960px;width:24px;height:24px;background:rgba(255, 255, 255, 1)">
      <div style="position:absolute;left:-67px;top:9960px;width:24px;height:24px">
      <div style="min-width:24px;min-height:24px">
      <div style="min-width:12px;min-height:12px">
      
      
    </div>
      <div style="min-width:20px;min-height:20px">
      
      
    </div>
      <div style="min-width:24px;min-height:24px">Vector</div>
    </div>
    </div>
    </div>
      <div style="position:absolute;left:-72px;top:9993px;width:42px;height:16px;background:rgba(0, 0, 0, 1);font-family:'IBM Plex Sans Arabic', Tajawal, system-ui, sans-serif;font-weight:400;font-size:12px;line-height:16px;color:rgba(0, 0, 0, 1)">المسترجعه</div>
    </div>
  </div>
</template>

<script setup lang="ts">
</script>
