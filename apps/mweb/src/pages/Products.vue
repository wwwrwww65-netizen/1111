    ...
    <div class="w-full bg-white border-b border-gray-200 sticky top-0 z-40">
      <div class="h-14 px-3 flex items-center justify-between">
        ...
        <div class="flex-1 px-2">
          <div class="flex items-center bg-gray-100 rounded-full h-10 px-3">
            <div class="flex-1 flex items-center justify-start">
              <span class="text-[14px] text-gray-400 truncate">{{ promoWords[promoIndex] }}</span>
            </div>
            <div class="flex items-center gap-1">
              <button aria-label="كاميرا" class="w-8 h-8 flex items-center justify-center opacity-60">
                <Camera class="w-5 h-5 text-gray-500" />
              </button>
              <div class="h-8 px-4 rounded-full flex items-center justify-center shadow-sm" style="background-color:#8a1538">
        ...
        <div class="flex items-center gap-0.5">
          <button aria-label="المفضلة" class="w-9 h-9 flex items-center justify-center p-0">
            <Heart class="w-5 h-5 text-gray-700" />
          </button>
          <button aria-label="عربة التسوق" class="w-9 h-9 flex items-center justify-center p-0 relative">
      ...
      <div v-if="!compact" class="bg-white border-t border-gray-100">
        <div class="flex gap-2 overflow-x-auto no-scrollbar px-2 py-2 items-start">
          <button v-for="c in visibleCategories" :key="c.id" class="flex flex-col items-center min-w-[76px] pb-1" @click="onCategoryClick(c)">
            ...
            <span class="mt-1 text-[13px] text-gray-700 text-center leading-tight category-title">{{ c.label }}</span>
          </button>
        </div>
      </div>
      ...
      <div v-if="compact" class="bg-white border-t border-gray-100">
        <div class="flex gap-2 overflow-x-auto no-scrollbar px-2 py-1 items-center">
          <button v-for="c in compactCategories" :key="c.id" class="flex items-center gap-3 min-w-[140px] px-2 py-1 rounded-md hover:bg-gray-50" @click="onCategoryClick(c)">
            <div class="w-11 h-11 rounded-full bg-white border border-gray-200 flex items-center justify-center overflow-hidden shrink-0">
              <img :src="c.img" :alt="c.label" class="w-full h-full object-cover" />
            </div>
            <div class="text-right">
              <div class="text-[14px] text-gray-800 leading-tight truncate-2-lines max-w-[8rem]">{{ c.label }}</div>
            </div>
          </button>
        </div>
      </div>
      ...
      <div v-if="showHeaderFilters" class="bg-white border-t border-gray-100 px-2 py-2">
        <div class="flex items-center justify-between mb-2">
          <button @click="setFilter('recommend')" :class="['text-[13px]', activeFilter === 'recommend' ? 'text-black font-semibold' : 'text-gray-600']">التوصية</button>
          <button @click="setFilter('popular')" :class="['text-[13px]', activeFilter === 'popular' ? 'text-black font-semibold' : 'text-gray-600']">الأوسع انتشاراً</button>
          <button @click="togglePriceSort" class="flex items-center gap-1 text-[13px]" :class="['', activeFilter === 'price' ? 'text-black font-semibold' : 'text-gray-600']">
            السعر
          ...
          <button @click="setFilter('rating')" :class="['flex items-center gap-1 text-[13px]', activeFilter === 'rating' ? 'text-black font-semibold' : 'text-gray-600']">
            <Filter class="w-3.5 h-3.5" /> التصنيف
          </button>
        </div>
        <div class="flex gap-2 overflow-x-auto no-scrollbar py-1">
          <button v-for="f in ['الفئات','المقاس','اللون','المواد','الأسلوب']" :key="f" class="flex items-center gap-1 bg-[#f7f7f7] px-3 py-1.5 rounded-md border border-gray-200 text-[13px] text-gray-800 min-w-max">
