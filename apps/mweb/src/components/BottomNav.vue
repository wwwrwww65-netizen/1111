<template>
  <nav class="bottom-nav">
    <RouterLink to="/" :aria-current="active==='home' ? 'page' : undefined"><Icon name="home" /><span>الرئيسية</span></RouterLink>
    <RouterLink to="/categories" :aria-current="active==='categories' ? 'page' : undefined"><Icon name="grid" /><span>التصنيفات</span></RouterLink>
    <RouterLink to="/products?sort=new" :aria-current="active==='new' ? 'page' : undefined"><Icon name="sparkles" /><span>جديد</span></RouterLink>
    <RouterLink to="/cart" id="cart-target" class="cart-link" :aria-current="active==='cart' ? 'page' : undefined">
      <Icon name="cart" />
      <span class="badge" v-if="count>0">{{ count }}</span>
      <span>حقيبة التسوق</span>
    </RouterLink>
    <RouterLink to="/account" :aria-current="active==='account' ? 'page' : undefined"><Icon name="user" /><span>الحساب</span></RouterLink>
  </nav>
</template>
