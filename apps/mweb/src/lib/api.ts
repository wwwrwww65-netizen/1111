...
export async function apiPost<T = any>(path: string, body?: any, init?: RequestInit): Promise<T | null> {
  try {
    ...
    const res = await fetch(url, { method:'POST', credentials: isPublic ? 'omit' : 'include', headers: { 'Accept':'application/json', 'Content-Type':'application/json', 'X-Shop-Client':'mweb', ...(init?.headers||{}) }, body: body!=null ? JSON.stringify(body) : undefined, ...init })
    if (!res.ok) return null
    return await res.json()
  } catch {
    return null
