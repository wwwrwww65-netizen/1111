...
export default function OrdersPage(): JSX.Element {
  ...
  async function ship(orderId: string) {
    // ship action is removed, use out_for_delivery or process
    // Keeping function signature but changing behavior to out_for_delivery if needed, or just remove usage
    // For now, let's map it to out_for_delivery as a fallback or just log error
    console.warn('ship action is deprecated');
  }
  ...
  async function bulk(action: 'ship' | 'cancel') {
    ...
    for (const id of ids) {
      if (action === 'ship') {
        // Bulk ship -> Out for delivery
        await fetch(`${apiBase}/api/admin/status/change`, { method: 'POST', headers: { 'content-type': 'application/json', ...authHeaders() }, credentials: 'include', body: JSON.stringify({ entity: 'order', id, action: 'out_for_delivery' }) });
      }
      ...
      <main className="panel" style={{ padding: 16 }}>
        ...
        <FilterBar value={search} onChange={(v) => setSearch(v)} right={<div style={{ color: 'var(--sub)', fontSize: 12 }}>{pendingFilters ? '...تطبيق المرشحات' : `${total} نتيجة`}</div>}>
          <select value={status} onChange={(e) => setStatus(e.target.value)} className="select filter">
            ...
            <option value="OUT_FOR_DELIVERY">في الطريق إليك</option>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
          {[
            ...
            { k: 'PROCESSING', label: 'قيد التنفيذ' },
            { k: 'OUT_FOR_DELIVERY', label: 'في الطريق' },
        ...
        <ResponsiveTable
          ...
          renderCard={(o: any) => {
            const shippingState = o.shipments?.[0]?.status || (o.status === 'OUT_FOR_DELIVERY' ? 'IN_TRANSIT' : o.status === 'DELIVERED' ? 'DELIVERED' : '-');
            return (
...
function Progress({ status, shipmentStatus }: { status: string; shipmentStatus?: string }): JSX.Element {
  ...
  const current = (() => {
    const map: Record<string, number> = { PENDING: 0, PROCESSING: 1, OUT_FOR_DELIVERY: 2, DELIVERED: 3 };
