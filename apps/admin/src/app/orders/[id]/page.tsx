...
export default function OrderDetailPage({ params }: { params: { id: string } }): JSX.Element {
  ...
  const recName = (order as any).shippingAddressSnapshot?.fullName ?? order.shippingAddress?.fullName ?? order.address?.fullName ?? order.user?.name ?? '-';
  const recPhone = (order as any).shippingAddressSnapshot?.phone ?? order.shippingAddress?.phone ?? order.address?.phone ?? order.user?.phone ?? '-';

  const normalizeAddress = (o: any) => {
    const snap = o.shippingAddressSnapshot || o.shippingAddress || o.address || {};
    const isNewFormat = !!snap.city;
    const realArea = snap.area || (isNewFormat ? snap.city : (snap.details || '').split(' - ')[0]);

    let realLandmarks = snap.landmarks;
    if (!realLandmarks) {
      realLandmarks = isNewFormat ? snap.details : (snap.details || '').split(' - ').slice(1).join(' - ');
    }

    // Safety: Strip Area from Landmarks if duplicated
    ...
    return {
      ...
      state: snap.state || snap.province,
      area: realArea,
      street: snap.street,
  ...
  const recLine = [addr.country, addr.state, addr.area, addr.street, addr.landmarks].filter(Boolean).join(' / ');
  return (
