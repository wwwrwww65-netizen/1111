...
export default function ClicksReport(): JSX.Element {
  ...
  const [rows, setRows] = React.useState<Array<{ target:string; clicks:number }>>([]);
  async function load(){
    const r = await safeFetchJson<{ ok:boolean; clicks:any[] }>(buildUrl(`${apiBase}/api/admin/analytics/ia/clicks`, { from, to }));
    if (r.ok) setRows((r.data.clicks||[]).map((x:any)=> ({ target:String(x.target||'-'), clicks:Number(x.clicks||0) })));
  }
  React.useEffect(()=>{ load().catch(()=>{}); },[apiBase, from, to]);
  return (
    <main>
      <IndependentNav />
      <h1 style={{ marginBottom:12 }}>النقرات</h1>
      ...
      <div className="panel" style={{ padding:12 }}>
        <div style={{ overflowX:'auto' }}>
          <table className="table"><thead><tr><th>الهدف (href/selector)</th><th>النقرات</th></tr></thead><tbody>
            {rows.map(r=> (<tr key={r.target}><td className="truncate" style={{ maxWidth:520, direction:'ltr' }}>{r.target||'-'}</td><td>{r.clicks.toLocaleString()}</td></tr>))}
