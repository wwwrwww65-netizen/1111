...
export default function RealtimePage(): JSX.Element {
  ...
  function fmtTime(d:string){ try{ const dt=new Date(d); return tz==='utc'? dt.toUTCString().slice(17,25) : dt.toLocaleTimeString(); }catch{ return d; } }
  async function load(){
    ...
    if (r.ok) { setRt({ windowMin:r.data.windowMin, online:r.data.online, metrics: r.data.metrics||{} }); setErr(''); } else { setErr(r.message||'failed'); }
  }
  ...
  async function loadSessions(){
    try{
      const rec = await safeFetchJson<{ ok:boolean; sessions:any[] }>(buildUrl(`${apiBase}/api/admin/analytics/realtime/sessions`, { windowMin: String(win) }));
      if (!rec.ok) { setSessions([]); return; }
      setSessions((rec.data.sessions||[]).slice(0,200));
    }catch{ setSessions([]); }
  }
  ...
  return (
    <main>
      ...
      <div style={{ display:'flex', alignItems:'center', gap:8, marginBottom:10 }}>
        ...
        <select className="input" value={tz} onChange={e=> setTz(e.target.value)} style={{ width:120 }}>
          <option value="local">محلي</option>
          <option value="utc">UTC</option>
        </select>
      </div>
      {!!err && errorView(err, ()=> { load().catch(()=>{}); loadSessions().catch(()=>{}); })}
