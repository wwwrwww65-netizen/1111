...
export default function FormsReport(): JSX.Element {
  ...
  const [rows, setRows] = React.useState<Array<{ form:string; submits:number }>>([]);
  async function load(){
    const r = await safeFetchJson<{ ok:boolean; forms:any[] }>(buildUrl(`${apiBase}/api/admin/analytics/ia/forms`, { from, to }));
    if (r.ok) setRows((r.data.forms||[]).map((x:any)=> ({ form:String(x.form||'-'), submits:Number(x.submits||0) })));
  }
  React.useEffect(()=>{ load().catch(()=>{}); },[apiBase, from, to]);
  return (
    <main>
      <IndependentNav />
      <h1 style={{ marginBottom:12 }}>النماذج</h1>
      ...
      <div className="panel" style={{ padding:12 }}>
        <div style={{ overflowX:'auto' }}>
          <table className="table"><thead><tr><th>النموذج</th><th>الإرسال</th></tr></thead><tbody>
            {rows.map(r=> (<tr key={r.form}><td className="truncate" style={{ maxWidth:520 }}>{r.form||'-'}</td><td>{r.submits.toLocaleString()}</td></tr>))}
