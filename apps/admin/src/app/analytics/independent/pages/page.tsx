...
export default function PagesReport(): JSX.Element {
  ...
  const [rows, setRows] = React.useState<Array<{ name:string; url:string; views:number; visitors:number; sessions:number }>>([]);
  const [q, setQ] = React.useState('');
  async function load(){
    const r = await safeFetchJson<{ ok:boolean; pages:any[] }>(buildUrl(`${apiBase}/api/admin/analytics/ia/pages`, { from, to }));
    if (r.ok){ setRows((r.data.pages||[]).map((x:any)=> ({ name: x.product?.name || (String(x.url||'').split('/').filter(Boolean).pop()||'-'), url:String(x.url||'-'), views:Number(x.views||0), visitors:Number(x.visitors||0), sessions:Number(x.sessions||0) }))); }
  }
    ...
    <main>
      ...
      <div className="panel" style={{ padding:12 }}>
        <div style={{ overflowX:'auto' }}>
          <table className="table"><thead><tr><th>الاسم</th><th>الرابط</th><th>المشاهدات</th><th>الزوار</th><th>الجلسات</th></tr></thead><tbody>
            {filtered.map(r=> (<tr key={r.url}><td className="truncate" style={{maxWidth:220}}>{r.name}</td><td style={{maxWidth:520, direction:'ltr'}} className="truncate">{r.url}</td><td>{r.views.toLocaleString()}</td><td>{r.visitors.toLocaleString()}</td><td>{r.sessions.toLocaleString()}</td></tr>))}
