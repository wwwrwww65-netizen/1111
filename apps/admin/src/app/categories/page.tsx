...
export default function CategoriesPage(): JSX.Element {
  ...
  async function loadTree(){
    const res = await fetch(`${apiBase}/api/admin/categories/tree`, { credentials:'omit', cache:'no-store', headers: { 'authorization': authHeaders().Authorization || '' } });
  ...
  async function add(){
    try {
      ...
      if (finalImage && finalImage.startsWith('data:')) {
        try {
          const up = await fetch(`${apiBase}/api/admin/media/upload`, { method:'POST', credentials:'omit', headers:{ 'content-type':'application/json', ...authHeaders() }, body: JSON.stringify({ filename: `cat-${Date.now()}.png`, contentType: 'image/png', base64: finalImage }) });
      ...
      const res = await fetch(`${apiBase}/api/admin/categories`, { method:'POST', headers:{ 'content-type':'application/json', 'authorization': authHeaders().Authorization || '' }, credentials:'omit', cache:'no-store', body: JSON.stringify(payload) });
      if (!res.ok) {
  ...
  async function remove(id:string){
    ...
    const res = await fetch(`${apiBase}/api/admin/categories/${id}`, { method:'DELETE', credentials:'omit', headers: { 'authorization': authHeaders().Authorization || '' } });
            ...
            <div onDragOver={(e)=>{ e.preventDefault(); }} onDrop={async (e)=>{
              ...
              try {
                ...
                reader.onload = async ()=> {
                  ...
                  try {
                    const resp = await fetch(`${apiBase}/api/admin/media/upload`, { method:'POST', credentials:'omit', headers:{ 'content-type':'application/json', ...authHeaders() }, body: JSON.stringify({ filename: f.name, contentType: f.type, base64: data }) });
                    if (resp.ok) {
