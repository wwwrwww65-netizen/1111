...
export default function MobileMedia(): JSX.Element {
  ...
  return (
    <main className="grid" style={{ gap:12 }}>
      <div className="panel" style={{ padding:12 }}>
        <div style={{ fontWeight:800, marginBottom:8 }}>الوسائط</div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'repeat(3, 1fr)', gap:8 }}>
          {rows.map((a)=> (
            <div key={a.id} style={{ border:'1px solid #1c2333', borderRadius:8, overflow:'hidden' }}>
              <img src={a.url} alt={a.alt||''} style={{ width:'100%', height:96, objectFit:'cover' }} />
            </div>
          ))}
        </div>
      </div>
