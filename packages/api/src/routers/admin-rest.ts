...
adminRest.post('/categories/page/import-default', async (req, res) => {
  try{
    ...
    const promoBanner = { enabled: true, image: 'https://csspicker.dev/api/image/?q=women+fashion+banner&image_type=photo', title: '???? ????? ??????', href: '/products' };
    ...
    const women = [
      mk('women-new','?????? ??','new women fashion','????'),
      mk('women-dresses','??????','dresses'),
      mk('women-long-dresses','?????? ?????','long dresses'),
      mk('women-tops','????? ?????','women tops'),
      mk('women-tshirts','?? ??????','women tshirts'),
      mk('women-blouses','?????','blouses'),
      mk('women-bottoms','????? ?????','women bottoms'),
      mk('women-skirts','??????','skirts'),
      mk('women-pants','??????','women pants'),
      mk('women-knits','??????','knit wear'),
      mk('women-sweaters','???????','sweaters'),
      mk('women-sets','???? ?????','matching sets'),
    ];
    const men = [
      mk('men-new','???? ?????','men fashion new','????'),
      mk('men-shirts','?????','men shirts'),
      mk('men-tshirts','????????','men tshirts'),
      mk('men-pants','??????','men pants'),
      mk('men-hoodies','?????','men hoodies'),
      mk('men-jackets','???????','men jackets'),
      mk('men-shoes','????? ??????','men shoes'),
      mk('men-accessories','????????? ??????','men accessories'),
    ];
    const kids = [
      mk('kids-new','???? ?????','kids fashion new','????'),
      mk('kids-girls','????? ????','girls clothing'),
      mk('kids-boys','????? ?????','boys clothing'),
      mk('kids-baby','????? ???','baby clothing'),
      mk('kids-shoes','????? ?????','kids shoes'),
      mk('kids-toys','?????','kids toys'),
    ];
    const plus = [
      mk('plus-women','?????? ????? ????','plus size women'),
      mk('plus-men','?????? ????? ????','plus size men'),
      mk('plus-dresses','?????? ?????','plus size dresses'),
      mk('plus-activewear','????? ??????','plus size activewear'),
    ];
    const home = [
      mk('home-decor','????? ?????','home decor'),
      mk('home-kitchen','????? ????','kitchen tools'),
      mk('home-bedding','???????','bedding'),
      mk('home-pets','???????? ???????','pet supplies'),
      mk('home-storage','????? ??????','storage organization'),
    ];
    const beauty = [
      mk('beauty-makeup','?????','makeup'),
      mk('beauty-skincare','??????? ???????','skincare'),
      mk('beauty-haircare','??????? ??????','haircare'),
      mk('beauty-fragrance','????','perfume'),
      mk('beauty-nails','??????? ????????','nail care'),
      mk('beauty-tools','????? ?????','beauty tools'),
    ];
    const suggestions = [
      mk('sug-accessories','????????? ?????','fashion accessories'),
      mk('sug-kids','????? ????? ?????','kids comfortable clothing'),
      mk('sug-sports','????? ??????','sports equipment'),
      mk('sug-bags','????? ?????','stylish bags'),
      mk('sug-shoes','????? ?????','comfortable shoes'),
      mk('sug-jewelry','???????','jewelry'),
    ];
    const makeItem = (label:string, cats:any[])=> ({ label, grid:{ mode:'explicit', categories: cats }, promoBanner:{ enabled:false }, suggestions:{ enabled:true, title:'???? ????? ??? ?????', items: suggestions } });
    const tabs = [
      { key:'all', label:'??', grid:{ mode:'explicit', categories: [] as any[] }, sidebarItems: [ makeItem('???????', [...women.slice(0,4), ...men.slice(0,4)]) ] },
      { key:'women', label:'????', featured: women, grid:{ mode:'explicit', categories: women }, sidebarItems: [ makeItem('????', women) ] },
      { key:'kids', label:'?????', featured: kids, grid:{ mode:'explicit', categories: kids }, sidebarItems: [ makeItem('????', kids) ] },
      { key:'men', label:'????', featured: men, grid:{ mode:'explicit', categories: men }, sidebarItems: [ makeItem('????', men) ] },
      { key:'plus', label:'?????? ?????', featured: plus, grid:{ mode:'explicit', categories: plus }, sidebarItems: [ makeItem('????', plus) ] },
      { key:'home', label:'?????? + ????????? ???????', featured: home, grid:{ mode:'explicit', categories: home }, sidebarItems: [ makeItem('????', home) ] },
      { key:'beauty', label:'?????', featured: beauty, grid:{ mode:'explicit', categories: beauty }, sidebarItems: [ makeItem('????', beauty) ] },
    ];
    const sidebar = [
      { label:'???? ??', tabKey:'all' },
      { label:'????? ??????', tabKey:'women' },
