...
adminRest.post('/badges', async (req, res) => {
  try {
    const u = (req as any).user; if (!(await can(u.userId, 'users.manage'))) return res.status(403).json({ error: 'forbidden' }); await ensureP2Schemas();
...
adminRest.post('/wallet/entry', async (req, res) => {
  try {
    const u = (req as any).user; if (!(await can(u.userId, 'finance.expenses.update'))) return res.status(403).json({ error: 'forbidden' }); await ensureP2Schemas();
...
adminRest.get('/wallet/:userId/balance', async (req, res) => {
  try {
    const { userId } = req.params; await ensureP2Schemas();
...
adminRest.post('/fx/rate', async (req, res) => {
  try {
    const u = (req as any).user; if (!(await can(u.userId, 'settings.manage'))) return res.status(403).json({ error: 'forbidden' }); await ensureP2Schemas();
...
adminRest.get('/fx/convert', async (req, res) => {
  try {
    await ensureP2Schemas(); const from = String(req.query.from || 'USD').toUpperCase(); const to = String(req.query.to || 'USD').toUpperCase(); const amount = Number(req.query.amount || 0);
...
adminRest.post('/affiliate/click', async (req, res) => {
  try {
    await ensureP2Schemas(); const { code } = req.body || {}; if (!code) return res.status(400).json({ error: 'code_required' });
...
adminRest.post('/affiliate/convert', async (req, res) => {
  try {
    await ensureP2Schemas(); const { code, orderId, amount } = req.body || {}; if (!code || !orderId || !amount) return res.status(400).json({ error: 'code_orderId_amount_required' });
...
adminRest.post('/drivers', async (req, res) => {
  try {
    const u = (req as any).user; if (!(await can(u.userId, 'drivers.create'))) return res.status(403).json({ error: 'forbidden' });
    await ensureDriversSchema();
...
adminRest.post('/carriers', async (req, res) => {
  try {
    const u = (req as any).user; if (!(await can(u.userId, 'carriers.create'))) return res.status(403).json({ error: 'forbidden' });
  ...
  try {
    ...
    if (rulesStrict) {
      try {
        ...
        const loadNlpConfig = () => {
          try {
            const fs = require('fs'); const path = require('path'); const base = process.env.NLP_CONFIG_DIR || path.join(process.cwd(), 'config', 'nlp');
...
adminRest.post('/geo/countries', async (req, res) => {
  ...
  try {
    const data = schema.parse(req.body || {}); const row = await db.country.create({ data });
...
adminRest.put('/geo/countries/:id', async (req, res) => {
  ...
  try {
    const prev = await db.country.findUnique({ where: { id } }); const d = schema.parse(req.body || {}); const row = await db.country.update({ where: { id }, data: d });
...
adminRest.delete('/geo/countries/:id', async (req, res) => {
  const { id } = req.params; try {
    const prev = await db.country.findUnique({ where: { id } }); await db.country.delete({ where: { id } });
    const code = toCode(prev?.code) || toCode(prev?.name); if (code) await removeCountryCodeFromZones(code); res.json({ ok: true });
  } catch (e: any) { res.status(400).json({ ok: false, error: e.message || 'country_delete_failed' }); }
});
...
adminRest.post('/geo/cities', async (req, res) => {
  ...
  try {
    const data = schema.parse(req.body || {}); const row = await db.city.create({ data });
...
adminRest.put('/geo/cities/:id', async (req, res) => {
  ...
  try {
    const prev = await db.city.findUnique({ where: { id } }); const d = schema.parse(req.body || {}); const row = await db.city.update({ where: { id }, data: d });
...
adminRest.delete('/geo/cities/:id', async (req, res) => {
  const { id } = req.params; try {
    const prev = await db.city.findUnique({ where: { id } }); const ctry = prev ? await db.country.findUnique({ where: { id: prev.countryId } }) : null; const code = toCode(ctry?.code) || toCode(ctry?.name);
    await db.city.delete({ where: { id } }); if (code && prev) await removeCityFromZoneByCode(code, prev.name); res.json({ ok: true });
  } catch (e: any) { res.status(400).json({ ok: false, error: e.message || 'city_delete_failed' }); }
});
...
adminRest.post('/geo/areas', async (req, res) => {
  ...
  try {
    const data = schema.parse(req.body || {}); const row = await db.area.create({ data });
...
adminRest.put('/geo/areas/:id', async (req, res) => {
  ...
  try {
    const prev = await db.area.findUnique({ where: { id } }); const d = schema.parse(req.body || {}); const row = await db.area.update({ where: { id }, data: d });
...
adminRest.delete('/geo/areas/:id', async (req, res) => {
  const { id } = req.params; try {
    const prev = await db.area.findUnique({ where: { id } }); const city = prev ? await db.city.findUnique({ where: { id: prev.cityId } }) : null; const ctry = city ? await db.country.findUnique({ where: { id: city.countryId } }) : null; const code = toCode(ctry?.code) || toCode(ctry?.name);
...
adminRest.post('/reviews/:id/approve', async (req, res) => {
  try {
    const u = (req as any).user; if (!(await can(u.userId, 'reviews.write'))) return res.status(403).json({ error: 'forbidden' });
...
adminRest.get('/analytics/products/table', async (req, res) => {
  try {
    ...
    // WHERE للـ events
    const evConds: string[] = [`(properties->>'productId') IS NOT NULL`];
    const evArgs: any[] = [];
    let argIdx = 1;
    ...
    const whereEv = evConds.join(' AND ');

    // الأعلى مشاهدة (يدعم legacy viewcontent)
    const viewsTop: any[] = await db.$queryRawUnsafe(`
      SELECT (properties->>'productId') as "productId", COUNT(1) as views
      FROM "Event"
      WHERE name IN ('product_view','viewcontent') AND ${whereEv}
      GROUP BY 1 ORDER BY views DESC LIMIT ${limit}
    `, ...evArgs);
    const ids = viewsTop.map(r => String(r.productId));
    if (!ids.length) { return res.json({ ok: true, rows: [] }); }

    // add_to_cart لنفس المعرّفات
    const whereEvIds = whereEv + ` AND (properties->>'productId') = ANY($${argIdx})`;
    const evArgsIds = [...evArgs, ids];
    const atc: any[] = await db.$queryRawUnsafe(`
      SELECT (properties->>'productId') as "productId", COUNT(1) as add_to_cart
      FROM "Event"
      WHERE name='add_to_cart' AND ${whereEvIds}
      GROUP BY 1
    `, ...evArgsIds);

    // مشتريات لنفس المعرّفات
...
adminRest.get('/analytics/ia/kpis', async (req, res) => {
  try {
    ...
    // distinctVisitors => عدد الأشخاص (فريد) ضمن الفترة المحددة
    return res.json({ ok: true, kpis: { visitors: visitorsTotal, totalVisitors: visitorsTotal, visitorsWindow, distinctVisitors: visitorsWindow, views, pageViews: views, sessions, avgSessionDurationSec, bounceRate, viewsPerSession } });
...
adminRest.get('/analytics/ia/users', async (req, res) => {
  try {
    ...
    const rows: any[] = await db.$queryRawUnsafe(`
      WITH vs AS (
        SELECT "userId" as uid, id as sid, "firstSeenAt" as fs, "lastSeenAt" as ls
        FROM "VisitorSession" WHERE "firstSeenAt" BETWEEN $1 AND $2 AND "userId" IS NOT NULL
      ),
      ev AS (
        SELECT COALESCE("userId", properties->>'userId') AS uid, COALESCE("sessionId", properties->>'sessionId') as sid, MIN("createdAt") as fs, MAX("createdAt") as ls
      ...
      lastSess AS (
        SELECT DISTINCT ON (u.uid) u.uid, s.country, s.city, s."ipHash"
        FROM unioned u
        LEFT JOIN "VisitorSession" s ON s.id = u.sid
        ORDER BY u.uid, u.ls DESC
      )
      SELECT u.id, u.name, u.email, u.phone,
             COUNT(DISTINCT unioned.sid) AS sessions,
             MIN(unioned.fs) AS firstSeen,
             MAX(unioned.ls) AS lastSeen,
             COALESCE(lastSess.country,'') AS country,
             COALESCE(lastSess.city,'') AS city,
             COALESCE(lastSess."ipHash",'') AS ip
      FROM unioned
      JOIN "User" u ON u.id = unioned.uid
      LEFT JOIN lastSess ON lastSess.uid = u.id
      GROUP BY u.id, u.name, u.email, u.phone, lastSess.country, lastSess.city, lastSess."ipHash"
...
adminRest.get('/analytics/ia/user/:id', async (req, res) => {
  try {
    ...
    // اجلب أحداث المستخدم + أي أحداث لجلسات مرتبطة بالمستخدم في VisitorSession حتى لو لم تحمل userId
    const events: any[] = await db.$queryRawUnsafe(`
      WITH sess AS (
        SELECT id FROM "VisitorSession" WHERE "userId" = $1
      )
      SELECT id, "createdAt", name, "sessionId", "userId", "pageUrl", "referrer", "productId", device, os, browser, country, city, properties
      FROM "Event"
      WHERE ("userId" = $1 OR properties->>'userId' = $1 OR "sessionId" IN (SELECT id FROM sess) OR (properties->>'sessionId') IN (SELECT id FROM sess))
      ORDER BY "createdAt" ASC
    `, id);
    const pids = Array.from(new Set(events.map(e => String(e.productId || e?.properties?.productId || '')).filter(Boolean)));
    const prod = pids.length ? await db.product.findMany({ where: { id: { in: pids } }, select: { id: true, name: true, images: true } }) : [];
...
// Admin: ad-hoc events query for debugging (filters: sessionId, userId, limit)
adminRest.get('/events', async (req, res) => {
  try {
    const u = (req as any).user; if (!(await can(u.userId, 'analytics.read'))) return res.status(403).json({ error: 'forbidden' });
    const limit = Math.min(Math.max(Number(req.query.limit || 100), 1), 500);
    const userId = (req.query.userId as string | undefined) || undefined;
