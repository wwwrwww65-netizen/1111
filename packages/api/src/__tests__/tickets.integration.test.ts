...
describe('Tickets flow', () => {
  it('create -> assign -> comment -> close', async () => {
    ...
    const u = await db.user.upsert({ where: { email: 'admin@example.com' }, update: {}, create: { id: 'admin-e2e', email: 'admin@example.com', name: 'Admin', password: '$2a$12$abcdefghijklmnopqrstuv', role: 'ADMIN', isVerified: true } });
    ...
    const assign = await request(expressApp).post(`/api/admin/tickets/${id}/assign`).set('Authorization', `Bearer ${token}`).send({ userId: u.id });
