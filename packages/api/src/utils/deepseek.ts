...
export async function callDeepseekPreview(opts: {
  ...
  try {
    const systemPrompt = [
      'أنت مساعد متخصص في تحليل منتجات الملابس العربية.',
      '',
      '## 🚫 القيود الصارمة للأسماء:',
      '- الاسم يجب أن يكون طويلاً (8-12 كلمة) على الأقل.',
      '- ممنوع تماماً الأسماء المختصرة مثل "فستان" أو "جلابية" أو "لانجري" فقط.',
      '- يجب أن يحتوي الاسم على: [النوع] + [المادة] + [التصميم/القصّة] + [الميزات] + [عناصر إضافية متاحة مثل الأكمام/الخصر/القطع].',
      '',
      '## 📝 أمثلة على الأسماء المطلوبة:',
      '"فساتين سهرة طويل مطرز بتصميم أنيق ومناسب للمناسبات الخاصة والاحتفالات"',
      '"جلابية شيفون مبطن بتطريز سيم ذهبي وكرستال شرقي خليجي بربطة خصر وأكمام طويلة"',
      '"لانجيري 4 قطع نمري تول شفاف وصدريه شلش جبير أصلي مع حزام منفصل"',
      '',
      '## 📋 المطلوب:',
      '1) name: اسم طويل وشامل (8-12 كلمة) - إجباري، وسيرفض إن كان أقل.',
      '2) description: يجب أن يكون جدولاً بهذه الصفوف بالضبط وبهذا التنسيق (نقطة • ثم اسم الصف ثم النقطتين):',
      ...
      '3) price: استخرج فقط السعر الشمالي/القديم (عملة قديمة/ريال قديم/سعر شمال). تجاهل أي أسعار أخرى.',
      '4) colors: جميع الألوان المحددة.',
      '5) sizes: جميع المقاسات.',
      '6) keywords: 4-6 كلمات سيو.',
      '7) stock (اختياري): إذا وجد رقم واضح للمخزون.',
      '',
      'ممنوع استخدام أي تنسيق آخر غير النقاط أعلاه. أعد JSON فقط بهذه الحقول: {"name":string,"description":string,"price":number?,"colors":string[]?,"sizes":string[]?,"keywords":string[]?,"stock":number?}'
    ].join('\n')
    const payload = {
      model,
      messages: [
